...
<div id="board-sec01">
	...
	<div class="text button-wrap">
		<span class="btn_view bgnone"><a href="/notice" class="list ">목록으로 가기</a></span>
		<ul class="pageBtn">
			<c:choose>
				<c:when test="${pre_data.idx != null}">
				   <li><a href="/notice_view/?idx=${pre_data.idx}" class="prevText">이전</a></li>
				</c:when>
			</c:choose>
			
			<c:choose>
				<c:when test="${next_data.idx != null}">
					<li><a href="/notice_view/?idx=${next_data.idx}" class="nextText">다음</a></li>
				</c:when>
			</c:choose>
		</ul>
	</div>
	...
	<div class="notice-attach">
		<button type="button" class="notice-button-action notice-button-download" onclick="window.location.href='/upload/notice/${data.banner}'" title="" download>첨부파일 : ${data.ori_banner}</button>
	</div>
