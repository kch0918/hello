		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					if(result.list[i].is_top == 'Y')
					{
						inner += '	<li class="res_name" onclick="javascript:location.href=\'./notice_upload?idx='+result.list[i].idx+'\'">'+nullChk(result.list[i].main_title)+'<span class="ico_fix">고정</span></li>';
					}
					else
					{
						inner += '	<li class="res_name" onclick="javascript:location.href=\'./notice_upload?idx='+result.list[i].idx+'\'">'+nullChk(result.list[i].main_title)+'</li>';
					}
					inner += '	<li>'+nullChk(result.list[i].cate)+'</li>';
...
function setSort()
{
	for(var i = 0; i<document.getElementsByName('idxs').length; i++)
	{
		...
		$.ajax({
			...
			url : "/admin/setSort?tb=tbl_notice&sort="+i+"&idx="+idx,//호출 URL을 설정한다. GET방식일경우 뒤에 파라티터를 붙여서 사용해도된다.
...
<div id="container" class="report">
	<div class="contain_inner">
    	<div class="search">
    		<p>게시판 관리<span>게시글을 수정하고 관리합니다.</span></p>
    		<input type="button" class="enroll" value="등록" onclick="location.href='/admin/users/notice_upload'"/>
    		<div class="seacrh_info search_bot select_a">
    			<table>
                    <tr>
                        <td class="search_txt">게시판 선택</td>
                        <td>
                            <div class="m_line">
        ...
        <div class="search_con_wrap">
            ...
            <div>
                ...
                <ul>
                    ...
                    <li>NO.<span onclick="reSortAjax('sort_tbl_notice.idx')"><img src="/img/list_icon_down.png" id="sort_tbl_notice.idx"/></span></li>
                    <li>제목<span onclick="reSortAjax('sort_main_title')"><img src="/img/list_icon_down.png" id="sort_main_title"/></span></li>
                    <li>게시판<span onclick="reSortAjax('sort_cate')"><img src="/img/list_icon_down.png" id="sort_cate"/></span></li>
                    <li>작성자<span onclick="reSortAjax('sort_name')"><img src="/img/list_icon_down.png" id="sort_name"/></span></li>
                    <li>등록일<span onclick="reSortAjax('sort_tbl_notice.submit_date')"><img src="/img/list_icon_down.png" id="sort_tbl_notice.submit_date"/></span></li>
