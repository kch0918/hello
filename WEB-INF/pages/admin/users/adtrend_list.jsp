		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	</li>';
					inner += '	<li>'+[i+1]+'</li>';
					inner += '	<li class="res_name" onclick="javascript:location.href=\'./adtrend_upload?idx='+result.list[i].idx+'\'">'+nullChk(result.list[i].main_title)+'</li>';
...
function setSort()
{
	for(var i = 0; i<document.getElementsByName('idxs').length; i++)
	{
		...
		$.ajax({
			...
			url : "/admin/setSort?tb=tbl_trend&sort="+i+"&idx="+idx,//호출 URL을 설정한다. GET방식일경우 뒤에 파라티터를 붙여서 사용해도된다.
...
<div id="container" class="report">
	<div class="contain_inner">
    	<div class="search">
    		<p>광고트렌드 분석 관리<span>광고트렌드 분석 리스트입니다.</span></p>
    		<input type="button" class="enroll" value="등록" onclick="location.href='/admin/users/adtrend_upload'"/>
