...
<script>
	...
	//	if(order_by == "desc")
	//	{
	//		$("#sort_"+sort_type).html('<img src="/admin/img/icon_up.png">');
	//	}
	//	else if(order_by == "asc")
	//	{
	//		$("#sort_"+sort_type).html('<img src="/admin/img/icon_down.png">');
	//	}
	//	if(listSize != "")
	//	{
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td>'+result.list[i].contents+'</td>';
					inner += '	<td>'+result.list[i].pay+'</td>';
					inner += '	<td>'+result.list[i].min_pay+'</td>';
					inner += 	'<td>';
					inner += 		'<a class="mod-btn" href="/admin/vision_edit?idx='+result.list[i].idx+'">수정</a>';
					inner += 		'<a class="mod-btn" href="javascript:delete_proc('+result.list[i].idx+')">삭제</a>';
					inner += 	'</td>';
					inner += '</tr>';
				}
			}
	...
	<body>
		...
		<div class="table-top table">
			<div class="btn-area text-right">
				<div>			
					<a class="btn btn01" href="/admin/vision_ins">등록</a>
				</div>
			</div>
		</div>
		<div class="table-top table">
			<div class="search">
				<form id="shopForm" name="shopForm" method="get" action="vision_list">
		...
		<table class="list-table">
			<thead>
				<tr>
					...
					<th>광고내용<span id="sort_contents" onclick="reSelect('sort_contents')"><img src="/img/icon_down.png"/></span></th>
					<th>단가<span id="sort_country" onclick="reSelect('sort_pay')"><img src="/img/icon_down.png"/></span></th>
					<th>최소청약<span id="sort_country" onclick="reSelect('sort_min_pay')"><img src="/img/icon_down.png"/></span></th>
					<th>관리</th>
				</tr>
			</thead>
		...
		<form id="geoForm" name="geoForm" method="post" action="getGeo">
		</form>
